...
namespace Media.Http
{
    ...
    public sealed class HttpHeaders
    {
                        ...
                        case "source":
                            {
                                ...
                                {
                                    source = System.Net.Dns.GetHostEntry(sourcePart).AddressList[0];
                                }
                                continue;
                            }
                        case "ssrc":
        ...
        public static string ScaleHeader(double scale) { return scale.ToString(HttpMessage.VersionFormat, System.Globalization.CultureInfo.InvariantCulture); }

        public static bool TryParseSpeed(out double result) { throw new NotImplementedException(); }

        public static string SpeedHeader(double speed) { return speed.ToString(HttpMessage.VersionFormat, System.Globalization.CultureInfo.InvariantCulture); }
        ...
        public static bool TryParseTimestamp(string value, out string timestamp, out TimeSpan delay)
        {
            ...
            if (false == string.IsNullOrWhiteSpace(value))
            {
                ...
                else
                {
                    ...
                    string[] parts = timestampHeader.Split(HttpMessage.SpaceSplit, 2);
