...
namespace nVideo.Codecs.H264
{
    ...
    public static void predictWithMode(int mode, int[] residual, bool leftAvailable, bool topAvailable,
            bool topLeftAvailable, bool topRightAvailable, int[] leftRow, int[] topLine, int[] topLeft,
    ...
    private static void interpolateTop(bool topLeftAvailable, bool topRightAvailable, int[] topLeft,
            int[] topLine, int blkX, int blkY, int[] outs) {
        int a = topLeftAvailable ? topLeft[blkY >> 2] : topLine[blkX];

        outs[0] = (a + (topLine[blkX] << 1) + topLine[blkX + 1] + 2) >> 2;
        int i;
        for (i = 1; i < 7; i++)
            outs[i] = (topLine[blkX + i - 1] + (topLine[blkX + i] << 1) + topLine[blkX + i + 1] + 2) >> 2;

        if (topRightAvailable) {
            for (; i < 15; i++)
                outs[i] = (topLine[blkX + i - 1] + (topLine[blkX + i] << 1) + topLine[blkX + i + 1] + 2) >> 2;
            outs[15] = (topLine[blkX + 14] + (topLine[blkX + 15] << 1) + topLine[blkX + 15] + 2) >> 2;
        } else {
            outs[7] = (topLine[blkX + 6] + (topLine[blkX + 7] << 1) + topLine[blkX + 7] + 2) >> 2;
            for (i = 8; i < 16; i++)
                outs[i] = topLine[blkX + 7];
        }
    }

    private static void interpolateLeft(bool topLeftAvailable, int[] topLeft, int[] leftRow, int blkY, int[] outs) {
        int a = topLeftAvailable ? topLeft[blkY >> 2] : leftRow[0];

        outs[0] = (a + (leftRow[blkY] << 1) + leftRow[blkY + 1] + 2) >> 2;
        for (int i = 1; i < 7; i++)
            outs[i] = (leftRow[blkY + i - 1] + (leftRow[blkY + i] << 1) + leftRow[blkY + i + 1] + 2) >> 2;

        outs[7] = (leftRow[blkY + 6] + (leftRow[blkY + 7] << 1) + leftRow[blkY + 7] + 2) >> 2;
    }

    private static int interpolateTopLeft(bool topAvailable, bool leftAvailable, int[] topLeft, int[] topLine,
    ...
    private static void predictDC(int[] residual, bool topLeftAvailable, bool topRightAvailable,
            bool leftAvailable, bool topAvailable, int[] topLeft, int[] leftRow, int[] topLine, int mbOffX,
            int blkX, int blkY) {
    ...
    private static void predictDiagonalDownLeft(int[] residual, bool topLeftAvailable, bool topAvailable,
            bool topRightAvailable, int[] topLeft, int[] topLine, int mbOffX, int blkX, int blkY) {
        interpolateTop(topLeftAvailable, topRightAvailable, topLeft, topLine, mbOffX + blkX, blkY, topBuf);
    ...
    private static void predictDiagonalDownRight(int[] residual, bool topRightAvailable, int[] topLeft,
    ...
    private static void predictVerticalRight(int[] residual, bool topRightAvailable, int[] topLeft, int[] leftRow,
    ...
    private static void predictHorizontalDown(int[] residual, bool topRightAvailable, int[] topLeft, int[] leftRow,
            int[] topLine, int mbOffX, int blkX, int blkY) {
        interpolateTop(true, topRightAvailable, topLeft, topLine, mbOffX + blkX, blkY, topBuf);
        interpolateLeft(true, topLeft, leftRow, blkY, leftBuf);
        int tl = interpolateTopLeft(true, true, topLeft, topLine, leftRow, mbOffX, blkX, blkY);
    ...
    private static void predictVerticalLeft(int[] residual, bool topLeftAvailable, bool topRightAvailable,
            int[] topLeft, int[] topLine, int mbOffX, int blkX, int blkY) {
        interpolateTop(topLeftAvailable, topRightAvailable, topLeft, topLine, mbOffX + blkX, blkY, topBuf);
    ...
    private static void predictHorizontalUp(int[] residual, bool topLeftAvailable, int[] topLeft, int[] leftRow,
            int mbOffX, int blkX, int blkY) {
        interpolateLeft(topLeftAvailable, topLeft, leftRow, blkY, leftBuf);
